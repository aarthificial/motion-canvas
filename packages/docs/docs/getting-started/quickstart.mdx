...
1. Go to the project directory (You should replace `<project-path>` with the
   path you picked during scaffolding):
   ```shell
   cd <project-path>
   ```
2. Install necessary dependencies:
   ...
   npm install
   ```
3. Finally, start the editor:
   ```shell
   npm run serve
   ```
   You may see a yellow error message in the terminal saying "Could not
   auto-determine entry point", but it's nothing important, and you can safely
